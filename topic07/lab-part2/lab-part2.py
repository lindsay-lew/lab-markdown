...
$ python3 -m doctest --verbose topic07/lab-part2/lab-part2.py
...
def string_contains_url(s):
    ...
    return "http://" in s_lower or "https://" in s_lower
...
def extract_TLD(domain):
    ...
    'pizza'
    '''
    return domain.split('.')[-1]  # Split on periods and return the last element
...
def remove_duplicates_from_list(xs):
    ...
    result = [xs[0]]  # Start with the first element
    for i in range(1, len(xs)):
        if xs[i] != xs[i - 1]:  # Only add if different from the previous element
            result.append(xs[i])
...
def remove_duplicate_words(s):
    ...
    '''
    words = s.split()  # Convert string to a list of words
    unique_words = remove_duplicates_from_list(words)  # Remove duplicate words
    return ' '.join(unique_words)  # Convert list back to string
...
def how_many_claremonts_in_str(s):
    ...
    4
    '''
    return s.lower().count('claremont')  # Convert to lowercase and count occurrences